...
const ComplexityRouterConfig: React.FC<ComplexityRouterConfigProps> = ({
  ...
  const applyTierRows = (rows: TierDraft[], fallbackTierId: string, base: ComplexityRouterConfigValue = value) => {
  ...
  const removeTierRow = (id: string) => {
    ...
    const remaining = tierRows.filter((row) => row.id !== id);
    const fallback_tier_id = remaining.some((row) => row.id === currentFallbackId)
      ? currentFallbackId
      : (remaining.find((row) => row.name === "MEDIUM") ?? remaining[0])?.id ?? "";
    onChange({
      ...value,
      ...(removed &&
        (TIER_KEYS as string[]).includes(removed.id) && {
  ...
  const restoreBuiltInTier = (tier: keyof ComplexityTiers) => {
    ...
    applyTierRows(restoredInCanonicalOrder, currentFallbackId);
  };

  // Ids are minted against the rows themselves rather than component state: this component
  ...
  const addCustomTier = () => {
    const taken = new Set(tierRows.map((row) => row.id));
    const id =
      Array.from({ length: tierRows.length + 1 }, (_, n) => `new-${n + 1}`).find(
        (candidate) => !taken.has(candidate),
      ) ?? `new-${tierRows.length + 1}`;
    applyTierRows([...tierRows, { id, name: "", definition: "", models: [] }], currentFallbackId);
  };

  // Per-model params are keyed by tier NAME upstream, so a model edit prunes that name's entries
  // and a rename moves them, keeping the params on the row the operator sees.
  const updateTierRow = (id: string, patch: Partial<Omit<TierDraft, "id">>) => {
    const row = tierRows.find((candidate) => candidate.id === id);
    const nextParams = (): TierModelParamsByTier | undefined => {
      const params = value.tier_model_params;
      if (!row || !params) return params;
      if (patch.models) return pruneTierModelParams(params, row.name.trim(), patch.models);
      if (patch.name === undefined || !params[row.name.trim()]) return params;
      const { [row.name.trim()]: moved, ...others } = params;
      return { ...others, [patch.name.trim()]: moved };
    };
    applyTierRows(
      tierRows.map((candidate) => (candidate.id === id ? { ...candidate, ...patch } : candidate)),
      currentFallbackId,
      { ...value, tier_model_params: nextParams() },
    );
  };
