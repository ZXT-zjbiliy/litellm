...
async def test_get_spend_for_model_budget_reads_the_configured_model_key(
    ...
    with patch.object(budget_limiter.dual_cache, "async_get_cache", return_value=50.0) as mock_get:
        ...
        assert spend == 50.0
        assert mock_get.call_args.kwargs["key"] == (f"{VIRTUAL_KEY_SPEND_CACHE_KEY_PREFIX}:test-key:gpt-4:1d")
...
async def test_get_spend_for_end_user_model_budget(budget_limiter):
    ...
    with patch.object(budget_limiter.dual_cache, "async_get_cache", return_value=50.0) as mock_get:
        ...
        assert spend == 50.0
        assert mock_get.call_args.kwargs["key"] == (f"{END_USER_SPEND_CACHE_KEY_PREFIX}:test-user:gpt-4:1d")
...
async def test_get_fallback_model_within_budget_skips_exhausted_fallback(
    ...
    async def _spend_for_model(entity_type, entity_id, resolved):
